...
def predict():
    ...
    input_features = pd.DataFrame([[age, sex, cp, trtbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal]],
                                  columns=['age', 'sex', 'cp', 'trtbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal'])

    # Make prediction
    prediction = model.predict(input_features)
    risk_level = "Heart attack risk is high" if prediction[0] == 0 else "Heart attack risk is low"
